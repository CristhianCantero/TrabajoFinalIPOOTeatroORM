...
create table if not exists cine(
	idCine bigint(11) NOT NULL AUTO_INCREMENT,
    idActividad bigint(11) NOT NULL,
    genero varchar(50) NOT NULL,
    paisOrigen varchar(50) NOT NULL,
    PRIMARY KEY(idCine, idActividad), 
    FOREIGN KEY(idActividad) REFERENCES actividad(idActividad)
    ON UPDATE CASCADE ON DELETE CASCADE
);

create table if not exists musical(
    idMusical bigint(11) NOT NULL AUTO_INCREMENT,
    idActividad bigint(11) NOT NULL,
    director varchar(50) NOT NULL,
    cantidadPersonasEscena int(5) NOT NULL,
    PRIMARY KEY(idMusical, idActividad), 
    FOREIGN KEY(idActividad) REFERENCES actividad(idActividad)
    ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE table if not exists obrateatro(
    idObraTeatro bigint(11) NOT NULL AUTO_INCREMENT,
    idActividad bigint(11) NOT NULL,
    PRIMARY KEY(idObraTeatro, idActividad),
    FOREIGN KEY(idActividad) REFERENCES actividad(idActividad)
    ON UPDATE CASCADE ON DELETE CASCADE
);
